...
def main():
    """
    All the main code will live inside this function.
    """
    ...
    for test_data_name in test_data_names:
        ...
        with torch.no_grad():
            ...
            for i, (lr_imgs, hr_imgs) in enumerate(test_loader):
                ...
                print(lr_imgs.shape)
                lr_imgs_cpu = lr_imgs.squeeze(0)
                print(f"Shape after squeeze: {lr_imgs_cpu.shape}")
                lr_imgs_cpu = lr_imgs_cpu.permute(1, 2, 0)
                print(f"Shape after permute: {lr_imgs_cpu.shape}")
                lr_imgs_cpu = lr_imgs_cpu.cpu().numpy()
                cv2.imshow('Low Resolution Image Input', lr_imgs_cpu)
                ...
                key = cv2.waitKey(0)
        if key == ord('q'): # Press 'q' to quit the whole evaluation
            break
...
# This is the gate. It makes sure the main() function only runs in the original script.
